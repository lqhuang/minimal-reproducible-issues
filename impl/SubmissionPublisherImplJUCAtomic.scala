...
object SubmissionPublisher {
  ...
  final class BufferedSubscription[T] private[SubmissionPublisher] (
    ...
    private var buffer = new AtomicReferenceArray[Object](initCapacity)
      ...
      if (cap > 0) {
        val added =
          if (size >= cap && cap < maxCapacity) // resize
            ...
            buffer.compareAndSet(index, null, item.asInstanceOf[AnyRef])
          else {
    ...
    private def startOnOffer(stat: Int): Int = {
      ...
      if ((_ctl & (Ctl.REQS | Ctl.ACTIVE)) == Ctl.REQS
          && {
            _ctl = ctl.getAndUpdate(c => c | Ctl.RUN | Ctl.ACTIVE)
    ...
    private def tryStart(): Unit =
      try {
        ...
        case exc @ (_: RuntimeException | _: Error) =>
          ctl.getAndUpdate(c => c | Ctl.ERROR | Ctl.CLOSED)
          throw exc
      }
    ...
    def startOnSignal(bits: Int): Unit =
      if ((ctl.get() & bits) != bits
          && (ctl.getAndUpdate(c => c | bits) & (Ctl.RUN | Ctl.CLOSED)) == 0)
        tryStart()
    ...
    def onError(exc: Throwable): Unit = {
      ...
      val _ctl = ctl.getAndUpdate(x => x | Ctl.ERROR | Ctl.RUN | Ctl.ACTIVE)
      if ((_ctl & Ctl.CLOSED) == 0) {
        if ((_ctl & Ctl.RUN) == 0)
          tryStart()
        else
          (0 until buffer.length()).foreach(i => buffer.set(i, null))
      }
    }
    ...
    def consume(): Unit = {
      ...
      while (!break) {
        ...
        else if ({ _demand = demand.get(); _demand == 0L}
            && ((_ctl & Ctl.REQS) != 0)) // exhausted demand
          ctl.weakCompareAndSetVolatile(_ctl, _ctl & ~Ctl.REQS): Unit
          ...
          if ({ empty = _tail == _head; empty } && (_ctl & Ctl.COMPLETE) != 0) {
            ...
            val bit =
              if ((_ctl & Ctl.ACTIVE) != 0)
                ...
                Ctl.RUN
            if (ctl.weakCompareAndSetVolatile(_ctl, _ctl & ~bit) && bit == Ctl.RUN)
              break = true
          }
        }
      }
    ...
    def subscribeOnOpen(sub: Flow.Subscriber[? >: T]): Unit =
      if ((ctl.get() & Ctl.OPEN) == 0
          && (ctl.getAndUpdate(c => c | Ctl.OPEN) & Ctl.OPEN) == 0) {
    ...
    def closeOnComplete(sub: Flow.Subscriber[? >: T]): Unit = {
      if ((ctl.getAndUpdate(c => c | Ctl.CLOSED) & Ctl.CLOSED) == 0)
        consumeComplete(sub)
    }
